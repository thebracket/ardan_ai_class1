...
manual/
  book.toml
  src/                  # edit here
  mermaid*.js           # vendored Mermaid runtime for mdbook-mermaid
...
Build or live-preview the manual (`mdbook` and `mdbook-mermaid` are installed; the
Mermaid runtime is vendored in `manual/`, so nothing is fetched at build time):
...
Diagrams in the manual are fenced ` ```mermaid ` blocks, not ASCII art.
`manual/mermaid.min.js` and `manual/mermaid-init.js` are the vendored runtime
(committed); regenerate them with `mdbook-mermaid install manual` if the toolchain
is upgraded.
