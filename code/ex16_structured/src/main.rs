...
impl ToolFactory for Tools {
    async fn call_tool(&mut self, tool_name: &str, arguments: &str) -> (String, String) {
        match tool_name {
            ...
            "get_weather" => {
                // Parse the city the model asked about. A real tool would call a
                // weather API here; we return a fixed, structured report so the
                // demo is deterministic.
                let city = serde_json::from_str::<serde_json::Value>(arguments)
                    .ok()
                    .and_then(|args| {
                        args.get("city")
                ...
                // The important part: the result is a JSON *object* full of
                // named fields, not a paragraph of prose for the model to parse.
                let report = serde_json::json!({
                    ...
                    "forecast": [
                        ...
                        },
                        {
                            ...
                            "high_c": 22,
                            "low_c": 15,
                            "precipitation_chance_percent": 70,
                            "conditions": "Showers"
                        },
                        {
