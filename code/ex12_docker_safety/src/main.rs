    ...
    async fn call_tool(&mut self, tool_name: &str, arguments: &str) -> (String, String) {
        match tool_name {
            ...
            "read_file" => {
                ...
                // A failed read is a *result* of the tool call, not a crash. We
                // hand the operating system's error straight back to the model
                // and let it explain the refusal to the user. Note that this is
                // the container refusing us - not our code deciding to stop.
            ...
            "bash" => {
                ...
                // wildcards, `rm -rf` - the lot. The container is what stops it
                // being a problem.
