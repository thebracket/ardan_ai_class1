...
pub struct ToolDefinition {
    ...
    /// `true` for tools with side effects (sending, writing, deleting). A
    /// privileged tool is refused if it was requested after the model read
    /// untrusted tool output. See `ChatSession::enforce_provenance`.
    pub privileged: bool,
}
...
pub struct ChatSession<TOOLS: ToolFactory> {
    ...
    tools: TOOLS,
    /// If set, every tool result is wrapped in an explicit provenance envelope
    /// before it reaches the model.
    explicit_provenance: bool,
    /// If set, a privileged tool call is refused whenever the model has read
    /// untrusted tool output since the last user message.
...
impl<TOOLS: ToolFactory> ChatSession<TOOLS> {
    ...
    /// Turn the provenance defences on. `explicit` wraps tool results in a
    /// provenance envelope; `enforced` also gates privileged tools once the
    /// model has read untrusted data.
    pub fn set_provenance(&mut self, explicit: bool, enforced: bool) {
        self.explicit_provenance = explicit;
        self.enforce_provenance = enforced;
    }

    /// Wrap a tool result in an explicit provenance envelope. This is the
    /// "tell the model where the text came from" defence - useful, but only a
    /// speed bump, because the model is still free to ignore it.
    ...
    fn build_message(&mut self, prompt: &str) -> anyhow::Result<serde_json::Value> {
        if !prompt.is_empty() {
            ...
            // A fresh instruction from the (trusted) user re-authorises
            // privileged tools.
            self.tainted = false;
        }
    ...
    pub async fn send_prompt(
        ...
        loop {
            ...
            for tool_call in &tool_calls {
                ...
                // Is this tool one that can actually *do* something?
                let privileged = self
                    .tools
                    .tools()
                    .iter()
                    .any(|tool| tool.name == tool_call.name && tool.privileged);

                // Enforced provenance: if we've read untrusted tool output and
                // the model now wants to pull a lever, refuse. It doesn't matter
                // whether the model *believes* the injected instruction - the
                // action simply isn't authorised by its provenance.
                if self.enforce_provenance && privileged && self.tainted {
                    println!(
                        "🛑 Blocked `{}`: a privileged tool cannot be triggered by untrusted tool output.",
                        tool_call.name
                    );
                    self.messages.push(ChatTurn::ToolCall {
                        id: tool_call.id.clone(),
                        content: json!({
                            "error": "blocked by provenance policy: this tool has side effects and \
                                      the request followed untrusted tool output. The user must \
                                      ask for it directly."
                        })
                        .to_string(),
                    });
                ...
                if self.explicit_provenance {
                    println!("🛡️  Wrapped in a provenance envelope before sending to the model.");
                }

                let content =
                    self.format_tool_result(&tool_call.name, json!({ (key): value }).to_string());
                self.messages.push(ChatTurn::ToolCall {
                    id: tool_call.id.clone(),
                    content,
                });
